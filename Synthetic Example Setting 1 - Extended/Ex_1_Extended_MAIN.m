...
%   for solving finite-horizon optimal control problems of SETTING 1. Please 
%   see Appendix C.1 of the following article for more details:
%   
...
%--------------------------------------------------------------------------
N_set = [11 21 41]; % grid sizes 11^2, 21^2, ...
N_star = 41; % for the reference in the error analysis
%--------------------------------------------------------------------------
...
for i_N = 1:length(N_set) % iteraton over grid sizes
    ...
    str = sprintf('Ex_1_Extended_Data_N%2.0f',N(1));
...
str = sprintf('Ex_1_Extended_Data_N%2.0f',N_star);
load(str)
...
% Data
error_dp_J = zeros(T,length(N_set)); % MAD b/w J_dp and J_star
error_cdp_J = zeros(T,length(N_set)); % MAD b/w J_cdp and J_star

for i_N = 1:length(N_set)
    
    N = N_set(i_N);
    str = sprintf('Ex_1_Extended_Data_N%2.0f',N);
    load(str)
...
% Saving the outputs:
str = sprintf('Ex_1_Extended_Analysis');
